...
const FileList = () => {
    ...
    const handleCheckbox = (item) => {
        ...
        if(item.target.checked){
            dispatch(addFiles(item.target.id));
        }else{
            dispatch(removeFiles(item.target.id));
        }
    ...
    <form className='flex flex-col w-90p h-1/2 m-auto rounded-xl p-2 border-2 overflow-hidden'>
        ...
        <table className='max-h-full max-w-full overflow-scroll block'>
            ...
            <tr className='grid grid-row grid-cols-7 gap-2 items-center h-20'>
                ...
                <input type="checkbox" name="miCheckbox" onChange={handleCheckbox} id="miCheckbox" className='hidden'/>
                <label for="miCheckbox" className='h-8 w-8 border-2 rounded-lg m-auto'></label> 
