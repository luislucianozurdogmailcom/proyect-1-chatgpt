...
const VerticalCarousel = ({ items }) => {
  ...
  return (
    <Slider {...settings} className='overflow-hidden h-full w-full'>
    {items.map((item,i) => (
      <div className='w-full items-center text-center h-20' key={i + '-item'}>
        <div className={`text-center h-full flex flex-row justify-between ${item.color}`} >
        {/* Componente con fotito de folder */}
          <div className='w-5/6 flex flex-row items-center overflow-hidden'>
            <img src={folder_3} className='m-5 h-14 w-14'></img>
            <span className='font-bold mr-1 text-md'>{item.private}</span> <p className='text-md'>{item.docs}</p>
          </div>
